...
class PairedImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return torch.tensor(mri, dtype=torch.float32, device=self.device).unsqueeze(0), torch.tensor(ct, dtype=torch.float32, device=self.device).unsqueeze(0)

batch_size = 16

# Move the model and data loaders to GPU if available
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

train_dataset = PairedImageDataset(train_data, device)
train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
val_dataset = PairedImageDataset(val_data, device)
